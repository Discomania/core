AT.prototype.atFormHelpers = {
    ...
    showSignUpLink: function(next_state){
        if  (AccountsTemplates.options.hideSignUpLink)
            return false;
        var state = next_state || this.state || AccountsTemplates.getState();
        return ((state === "signIn" && !Meteor.userId()) || state === "forgotPwd" || state === "resendVerificationEmail") && !AccountsTemplates.options.forbidClientAccountCreation;
    },
    ...
    showResendVerificationEmailLink: function(){
        ...
        return ((state === "signIn" && !Meteor.userId()) || state === "forgotPwd") && AccountsTemplates.options.showResendVerificationEmailLink;
    },
};
